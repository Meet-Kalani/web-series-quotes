...
const savedQuotesContainerElement = document.querySelector(
...
document.addEventListener("DOMContentLoaded", () => {
  if (!savedQuotes) {
    savedQuotesTitleElement.textContent =
      "No saved quotes yet. Save a quote and come back!";
    return;
  }
...
function displaySavedQuote() {
  savedQuotes.forEach((savedQuote) => {
    const savedQuoteElement = createHTMLElement(
      savedQuotesContainerElement,
      "div",
      "saved-quote"
    );

    const quoteTextElement = createHTMLElement(
      savedQuoteElement,
      "p",
      "quote-text"
    );

    const wrapperDiv = createHTMLElement(savedQuoteElement, "div");

    const quoteAuthorElement = createHTMLElement(
      wrapperDiv,
      "span",
      "quote-author"
    );

    const quoteSeriesElement = createHTMLElement(
      wrapperDiv,
      "span",
      "quote-series"
    );

    const shareQuoteElement = createHTMLElement(
      savedQuoteElement,
      "a",
      "share-link"
    );
